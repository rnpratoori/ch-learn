...
def plot_nn_output_vs_c_from_data(c_values, nn_output_values, ylabel, title, output_path, true_f_values=None, true_dfdc_values=None):
    """
    Plots nn output vs c from saved data.
    Optionally, also plots the true f or df/dc.
    """
    try:
        fig, ax = plt.subplots(figsize=(6, 4))
        ax.plot(c_values, nn_output_values, label=f"Learned {ylabel}")

        if true_f_values is not None:
            ax.plot(c_values, true_f_values, label="True f(c)", linestyle='--')

        if true_dfdc_values is not None:
            ax.plot(c_values, true_dfdc_values, label="True df/dc", linestyle='--')
...
def reproduce_plots(npz_path):
    ...
    # --- 2. Plot nn output vs c for all saved epochs ---
    print("\nGenerating nn output vs c plots for all saved epochs...")
    if 'c_values_nn' in data and 'all_nn_outputs' in data and 'nn_output_label' in data:
        ...
        # --- Calculate true f and df/dc ---
        N1, N2, chi = 5, 5, 1
        epsilon = 1e-10
        c_vals_safe = np.clip(c_vals, epsilon, 1 - epsilon)

        true_f = c_vals_safe * np.log(c_vals_safe) / N1 + (1 - c_vals_safe) * np.log(1 - c_vals_safe) / N2 + chi * c_vals_safe * (1 - c_vals_safe)
        true_dfdc = (1/N1 - 1/N2) + (np.log(c_vals_safe) / N1 - np.log(1-c_vals_safe) / N2) + chi * (1 - 2 * c_vals_safe)

        true_f_to_plot = None
        true_dfdc_to_plot = None
        if "Free Energy" in ylabel:
            true_f_to_plot = true_f
        elif "df/dc" in ylabel:
            true_dfdc_to_plot = true_dfdc
        ...
        for nn_output_data in all_nn_outputs:
            ...
            output_path = plot_output_dir / f"nn_output_vs_c_epoch_{epoch}.png"
            ...
            plot_nn_output_vs_c_from_data(
                ...
                title,
                output_path,
                true_f_values=true_f_to_plot,
                true_dfdc_values=true_dfdc_to_plot
            )
    ...
    if 'all_epochs_comparison_data' in data:
        ...
        for epoch_data in all_epochs_data:
            ...
            if plot_now:
                ...
                plot_spacetime_error_3d(
                    epoch + 1,
                    epoch_data['data'],
                    plot_output_dir / f"spacetime_error_3d{filename_suffix}.png"
                )
    else:
