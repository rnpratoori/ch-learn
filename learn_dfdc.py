...
def compute_loss_and_gradient(u_curr, target, device, truncation_modes=0, weight=1.0):
    ...
    # The loss compares spectra instead of raw DOF-by-DOF values.  Truncation,
    # when requested, restricts the objective to coarse morphology modes.
    fft_u = torch.fft.fft(u_tensor)
    fft_t = torch.fft.fft(t_tensor)
    
    if truncation_modes > 0:
        # Zero out higher-frequency modes.  This branch is intentionally 1D;
        # 2D/3D data would need a multidimensional truncation mask.
